...
static void PortalTransitionHiding(void)
{
        ...
        (gCurrentTemporality == TEMPORALITY_FUTURE && gGameMode.work1 == 0))
    {
        ...
        // Remove v-blank callback to not interfere with what we'll be doing, also to save performance
...
void PortalTransition(void)
{
    gGameMode.work1 -= DELTA_TIME;

    if (gGameMode.work1 != 0)
        return;
